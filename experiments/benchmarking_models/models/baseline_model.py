...
class BaselineBEPSModel:
    def __init__(emissions_path, timeline_path, building_data_path, fine_years):
        '''
    ...
    def _get_expected_baseline(building, year):
        '''
    ...
    def _get_expected_baseline_ghgi(building):
        '''
    ...
    def _fill_in_expected_baselines(year_low, year_high, input_df, output_df):
        '''
    ...
    def _get_city_ghgi(building, orig_building_data):
        year = building['year']
        ...
        first_use_ghgi = self._find_ghgi_standard(timeline, year, orig_building_row['LargestPropertyUseType OSE'], building['sq_ft_classification'])
        second_use_ghgi = self._find_ghgi_standard(timeline, year, orig_building_row['SecondLargestPropertyUseType OSE'], building['sq_ft_classification'])
        third_use_ghgi = self._find_ghgi_standard(timeline, year, orig_building_row['ThirdLargestPropertyUseType OSE'], building['sq_ft_classification'])
    ...
    def _get_compliant_ghgi(building):
        '''
    ...
    def _get_compliant_emissions(building):
        return building['compliant_ghgi'] * building['Total_sqft']

    def _get_compliance_status(building):
        year = building['year']
        sqft_class = building['sq_ft_classification']
        if pd.isna(self._find_ghgi_standard(year, building['LargestPropertyUseType OSE'], sqft_class)) and pd.isna(self._find_ghgi_standard(timeline, year, building['SecondLargestPropertyUseType OSE'], sqft_class)) and pd.isna(self._find_ghgi_standard(timeline, year, building['ThirdLargestPropertyUseType OSE'], sqft_class)): 
            return 'Not due yet'
    ...
    def _get_noncompliance_fines(building):
    ...
    def calculate_baseline_model(start_year, end_year):
        ...
        scen_calcs['city_ghgi_target'] = scen_calcs.apply(lambda building: self._get_city_ghgi(building, self.timeline, self.building_data), axis=1)
        scen_calcs['compliant_ghgi'] = scen_2_calcs.apply(lambda building: self._get_compliant_ghgi(building), axis=1)
        scen_calcs['compliant_emissions'] = scen_2_calcs.apply(lambda building: self._get_compliant_emissions(building), axis=1)
        scen_calcs['compliance_status'] = scen_2_calcs.apply(lambda building: self._get_compliance_status(building, self.timeline), axis=1)
        scen_calcs['compliance_fees'] = scen_2_calcs.apply(lambda building: self._get_noncompliance_fines(building, self.fines_years), axis=1)
